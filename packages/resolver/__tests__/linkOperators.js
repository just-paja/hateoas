...
describe('HAL link resolver with different kind of operators', () => {
  it('String expansion', () => {
    ...
    expect(
      getHalLink(links, 'edit', {
        bar: 'bar',
        foo: 'foo',
        param: ['multiple', 'params'],
      })
    ).toBe('http://example.com/api/users/bar/foo?param=multiple,params')
  })
  it('Unnamed operator expansion', () => {
    ...
    expect(
      getHalLink(links, 'edit', { bar: ['bar1', 'bar2'], foo: 'foo' })
    ).toBe('http://example.com/api/users/bar1/bar2/foo')
  })
  ...
  it('Empty unnamed operator expansion', () => {
    ...
    expect(getHalLink(links, 'edit', { bar: '', foo: '' })).toBe(
  ...
  it('Named operator expansion', () => {
    ...
    expect(getHalLink(links, 'edit', { param: ['p1', 'p2'] })).toBe(
  ...
  it('Multiple named operators expansion', () => {
    ...
    expect(
      getHalLink(links, 'edit', { bar: ['bar1', 'bar2'], param: 'p1' })
    ).toBe('http://example.com/api/users?param=p1&bar=bar1&bar=bar2')
  })
  it('Empty named operators expansion', () => {
    ...
    expect(getHalLink(links, 'edit', { bar: ['', ''], param: '' })).toBe(
